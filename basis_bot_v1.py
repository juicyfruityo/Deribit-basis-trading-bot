...
class BasisTradingBot:
    ...
    async def start_check_price(self):
        ...
        async with websockets.connect(self.uri) as websocket:
            self.logging_bot(f"Socket fro price is opened", False)
            msg_data = {
            ...
            while websocket.open:
                self.__preprocess_price(await websocket.recv())
            self.logging_bot(f"Socket fro price is closed", False)

    def __preprocess_price(self, message):
    ...
    async def __check_order(self, websocket):
        ...
        await websocket.send(json.dumps(msg_order_state))
        while websocket.open:
          message = await websocket.recv()
          data = json.loads(message)
          if data.get('error') or "order_id" in data['result']:
              break
        if data.get('error'):
            self.logging_bot(f'ERROR with get_order_state in __check_order err={data["error"]}')
            raise

        # Делаем маркет ордер для базового актива на величину исполненной заявки.
        if data['result']['order_state'] != 'open':
            self.logging_bot(f'In check order Making market order, amount = {data["result"]["filled_amount"]}', True)

            amount = data['result']['filled_amount'] - order_info['filled_amount']
            market_price = await self.__market_order(websocket, self.params['base_side'],
                                                    self.params['base_inst'], amount)
            self.current_orders[self.params['other_inst']]['filled_amount'] += amount

            self.logging_bot(f'amount={order_info["amount"]} filled_amount={data["result"]["filled_amount"]}')
            if data['result']['filled_amount'] == order_info['amount']:
